...
; Quiz 3 Vectors 
; Group Members: Dimitri Short, Kyle Dunn, Derek Diskant, Maryam Attalla, Gregori Presinal

; Thumbs up if largest and smallest vector element are abstracted
...
(define (Max V)
  (local [; Contract: (max-help(vectorof number)) -> max
          ...
          ; max is the highest valued number in the processed part of the vector [0...low]
...
(define (Lowest V)
  (local [; Contract (low-help(vectorof number)) -> lowest
          ...
          ; lowest is the lowest number in the processed part of the vector [0...low]
...
(define (max-and-min V sign)
  (local [; Contract: (vectorof number) (> or <) -> value 
